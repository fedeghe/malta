...
The _multi-mode_ purpose is guess what to launch Malta on more that one build in one command. In this case it take just one parameter that is the path to a json file which contains for each file the same informations. It uses as key the templateFile path and as value all other parameters space separated. E.g.:
...
here is possible to tell Malta to consider a different file for variables (default would be _templateFolded_/vars.json); if used the path specified must be relative to the execution folder.
